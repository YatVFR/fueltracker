(function(){
  ...
  const REV='v16.4.1-station-reliability-1';
  ...
  function handlePosition(pos){
    ...
    if(!nearby.length){candidateId='';candidateHits=0;clearOwnVisit();renderDiagnostics();return;}
    const unambiguous=nearby.length===1||(nearby[1].d-nearby[0].d>=40);if(!unambiguous){candidateId='';candidateHits=0;clearOwnVisit();renderDiagnostics();return;}
    const chosen=nearby[0].s;if(candidateId===chosen.id)candidateHits++;else{candidateId=chosen.id;candidateHits=1;}if(candidateHits<2){renderDiagnostics();return;}
    if(!confirmOriginalChoice(chosen.id))startOwnDwell(chosen);else{clearOwnVisit();document.getElementById('v160StationConfirm')?.remove();}
    renderDiagnostics();
  }
  ...
  document.addEventListener('change',e=>{if(e.target?.id==='v160Enabled'||e.target?.id==='v160Radius'||e.target?.id==='v160Dwell')setTimeout(restart,100);});
  document.addEventListener('fueltracker:pagechange',e=>{if(e.detail?.page==='settings')setTimeout(renderDiagnostics,100);});
  document.addEventListener('fueltracker:datachange',sync);
  window.addEventListener('focus',()=>{if(settings().enabled)restart();});
  window.addEventListener('pageshow',()=>{if(settings().enabled)restart();});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&settings().enabled)restart();});
  [300,900,1800].forEach(ms=>setTimeout(sync,ms));
  window.FuelTrackerStationReliabilityV16={revision:REV,start,stop,restart,testLocation,status:statusText};
})();