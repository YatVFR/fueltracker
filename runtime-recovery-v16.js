(function(){
  ...
  const REV='v16.3.4-runtime-recovery-1';
  const VERSION='v16.3.4 Maintenance Verification';
  const DISPLAY='16.3.4';
  const COMPAT='16.34';
  ...
  async function recover(){
    addCss('./mobile-header-fix.css','v1634Mobile');addCss('./garage-v15.css','v1634Garage');addCss('./maintenance-form-fix-v16.css','v1634MaintenanceForm');applyVersion();
    try{if(typeof renderAll==='function')renderAll();}catch(e){console.warn('v16.3.4 render recovery',e);}await sleep(80);
    if(!hasGarage()){await load('v1634GarageScript','./garage-v15.js?v=1634');try{if(typeof renderAll==='function')renderAll();}catch(e){}await sleep(80);}
    if(!document.getElementById('garageOverviewBox'))await load('v1634OverviewScript','./garage-overview-v15.js?v=1634');
    if(!document.getElementById('garageAnalyticsBox'))await load('v1634AnalyticsScript','./garage-analytics-v15.js?v=1634');
    if(!document.getElementById('odometerGrid')?.children.length)await load('v1634OdoScript','./odometer-live-v15.js?v=1634');
    if(!document.getElementById('v158PageNav'))await load('v1634NavScript','./navigation-v15-8.js?v=1634');
    await sleep(120);
    if(!window.FuelTrackerAutomation)await load('v1634AutomationScript','./automation-v16.js?v=1634');
    if(!window.FuelTrackerGarageMaintenanceV16)await load('v1634MaintenanceScript','./garage-maintenance-v16.js?v=1634');
    if(!window.FuelTrackerMaintenanceTrialV16)await load('v1634TrialScript','./maintenance-trial-v16.js?v=1634');
    if(!window.FuelTrackerUpdateStatusV16)await load('v1634UpdateStatusScript','./update-status-v16.js?v=1634');
