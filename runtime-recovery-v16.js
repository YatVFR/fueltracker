(function(){
  ...
  const REV='v16.3.6-runtime-recovery-1';
  const VERSION='v16.3.6 Backup Integrity';
  const DISPLAY='16.3.6';
  const COMPAT='16.36';
  ...
  async function recover(){
    addCss('./mobile-header-fix.css','v1636Mobile');addCss('./garage-v15.css','v1636Garage');addCss('./maintenance-form-fix-v16.css','v1636MaintenanceForm');applyVersion();
    try{if(typeof renderAll==='function')renderAll();}catch(e){console.warn('v16.3.6 render recovery',e);}await sleep(80);
    if(!hasGarage()){await load('v1636GarageScript','./garage-v15.js?v=1636');try{if(typeof renderAll==='function')renderAll();}catch(e){}await sleep(80);}
    if(!document.getElementById('garageOverviewBox'))await load('v1636OverviewScript','./garage-overview-v15.js?v=1636');
    if(!document.getElementById('garageAnalyticsBox'))await load('v1636AnalyticsScript','./garage-analytics-v15.js?v=1636');
    if(!document.getElementById('odometerGrid')?.children.length)await load('v1636OdoScript','./odometer-live-v15.js?v=1636');
    if(!document.getElementById('v158PageNav'))await load('v1636NavScript','./navigation-v15-8.js?v=1636');
    await sleep(120);
    if(!window.FuelTrackerAutomation)await load('v1636AutomationScript','./automation-v16.js?v=1636');
    if(!window.FuelTrackerGarageMaintenanceV16)await load('v1636MaintenanceScript','./garage-maintenance-v16.js?v=1636');
    if(!window.FuelTrackerMaintenanceTrialV16)await load('v1636TrialScript','./maintenance-trial-v16.js?v=1636');
    if(!window.FuelTrackerMaintenanceTypeV16)await load('v1636TypeScript','./maintenance-type-v16.js?v=1636');
    if(!window.FuelTrackerUpdateStatusV16)await load('v1636UpdateStatusScript','./update-status-v16.js?v=1636');
