(function(){
  ...
  const REV='v16.4.0-masterdb-current-1';
  const DISPLAY='16.4.0';
  ...
  function optNumber(v){if(v==null||v==='')return null;const n=Number(v);return Number.isFinite(n)?n:null;}
  function normalize(r){return {id:r?.id||'',dateTime:r?.dateTime||'',location:r?.location||r?.station||'',volume:Number(r?.volume)||0,cost:Number(r?.cost)||0,currency:String(r?.currency||'').toUpperCase(),fxRateSGDMYR:Number(r?.fxRateSGDMYR)>0?Number(r.fxRateSGDMYR):null,mileage:Number(r?.mileage)||0,fuelType:r?.fuelType||r?.fuelGrade||'',notes:r?.notes||'',pumpAmount:optNumber(r?.pumpAmount),unitPrice:optNumber(r?.unitPrice),discountType:r?.discountType||'none',discountValue:optNumber(r?.discountValue),discountAmount:optNumber(r?.discountAmount),discountLabel:r?.discountLabel||'',netPaid:optNumber(r?.netPaid),effectiveUnitPrice:optNumber(r?.effectiveUnitPrice),scanSource:r?.scanSource||null,scanTripMeter:optNumber(r?.scanTripMeter),scanOnboardConsumption:optNumber(r?.scanOnboardConsumption)};}
  ...
  async function exportDb(){
    ...
    const name=filename(p),payload={app:'Fuel Tracker Garage MasterDB',version:DISPLAY,schema:3,database:name,profile:profileMeta(p,d),exportedAt:new Date().toISOString(),entries:(d.records||[]).map(normalize)};
    try{
  ...
  function render(){
    ...
    const setting=db?.closest('.setting'),copy=setting?.querySelector('p');if(copy)copy.textContent=`This Garage vehicle owns an independent MasterDB. Current exports use v${DISPLAY} and include profile details, theme, odometer, fuel records and Smart Refuel Capture metadata.`;
