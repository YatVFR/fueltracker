const CACHE_NAME = 'astinafuel-offline-v13-2';
const APP_SHELL = [
  './',
  './index.html',
...
self.addEventListener('fetch', event => {
  ...
  if(healthURL.searchParams.has('health')){
    event.respondWith(fetch(event.request,{cache:'no-store'}).catch(()=>new Response('',{status:503,statusText:'Local server offline'})));
    return;
  }
