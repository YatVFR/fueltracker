(() => {
  ...
  const APP_SCOPE=location.pathname.includes('/carfueltracker/')?'car':'bike';
  const THEME_KEY='fuelTrackerTheme';
  const ONBOARDING_KEY=`fuelTrackerMasterDbOnboarded:${APP_SCOPE}`;
  const IDB_NAME='FuelTrackerSettings';
  const IDB_STORE='handles';
  const HANDLE_KEY=`masterDb:${APP_SCOPE}`;
  const THEMES=[
  ...
  const storageKey=()=>typeof STORAGE_KEY==='string'?STORAGE_KEY:(APP_SCOPE==='car'?'carFuelTrackerData':'fuelTrackerData');
  const masterFile=()=>typeof MASTER_FILE==='string'?MASTER_FILE:(document.getElementById('backupFileName')?.textContent?.trim()||(APP_SCOPE==='car'?'CarFuelData.json':'BikeFuelData.json'));
  ...
  async function importHandle(handle){
    if(handle.name && handle.name!==masterFile() && !confirm(`You selected ${handle.name}. This app normally uses ${masterFile()}. Continue anyway?`)) return false;
  ...
  async function locateMasterDb(){
    try{
      if(typeof window.showOpenFilePicker==='function'){
        ...
        if(!handle)return;const imported=await importHandle(handle);if(imported){await saveHandle(handle);markOnboarded();}
      }else{
